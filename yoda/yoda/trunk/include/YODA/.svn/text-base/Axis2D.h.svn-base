...
//        typedef typename Utils::sortedvector<Utils::sortedvector<BIN> > Bins;
        ...
        static inline pair<std::vector<double>, std::vector<double> > mkBinEdgesLinLin(double startX, double startY, double endX, double endY, size_t nbinsX, size_t nbinsY) {
            return make_pair(linspace(startX, endX, nbinsX), linspace(startY, endY, nbinsY));
        }

        static inline pair<std::vector<double>, std::vector<double> > mkBinEdgesLinLog(double startX, double startY, double endX, double endY, size_t nbinsX, size_t nbinsY) {
            return make_pair(linspace(startX, endX, nbinsX), logspace(startY, endY, nbinsY));
        }

        static inline pair<std::vector<double>, std::vector<double> > mkBinEdgesLogLog(double startX, double startY, double endX, double endY, size_t nbinsX, size_t nbinsY) {
            return make_pair(logspace(startX, endX, nbinsX), logspace(startY, endY, nbinsY));
        }

        static inline pair<std::vector<double>, std::vector<double> > mkBinEdgesLogLin(double startX, double startY, double endX, double endY, size_t nbinsX, size_t nbinsY) {
    ...
    private:
        ...
        bool _validateEdge(pair<pair<double,double>, pair<double,double> >& edge) {
            if(edge.first.first == edge.second.first) return _findCutsY(edge);
            else if(edge.first.second == edge.second.second) return _findCutsX(edge);
            else return false;
        }
        
        //TODO: Change those two to accept a vector instead of coords
        //TODO: These two can be sped up by breaking when second for goes past the measurement point
        ...
        void _dropEdge(pair<pair<double,double>, pair<double,double> >& edge) {
            ///TODO:Stuff to do when dropping edge that is a part of a conflicting rectangle
        }

        //Do not initiate a global sort when another edges were found on the same level
        void _addEdge(pair<pair<double,double>, pair<double,double> >& edge) {
            if(edge.first.first == edge.second.first) {
                ...
                for(int i=0; i < _binHashSparse.second.size(); i++) {
                    if(fuzzyEquals(_binHashSparse.second[i].first, edge.first.first)) {
                        _binHashSparse.second[i].second.push_back(
                            make_pair(edge.first.second, edge.second.second));
                        _binHashSparse.second[i].second.sort();
                        found = true;
                    }
                }
                if(!found) {
                    Utils::sortedvector<pair<double,double> > temp;
                ...
                for(int i=0; i < _binHashSparse.first.size(); i++) {
                    if(fuzzyEquals(_binHashSparse.first[i].first, edge.first.second)) {
                        _binHashSparse.first[i].second.push_back(
                            make_pair(edge.first.first, edge.second.first));
                ...
                if(!found) {
                    ...
                    _binHashSparse.first.push_back(make_pair(edge.first.second, temp));
                }
            }
        }
        ...
        void _mkAxis(const pair<vector<double>,vector<double> >& binedges) {
            ...
            std::sort(_cachedBinEdges.first.begin(), _cachedBinEdges.first.end());
            std::sort(_cachedBinEdges.second.begin(), _cachedBinEdges.second.end());
            _mkBinHash();
        }
    ...
    public:
        ...
        BIN& bin(std::pair<size_t, size_t> index) {
            bin(index.first, index.second);
        }
        const BIN& bin(std::pair<size_t, size_t> index) const {
            bin(index.first, index.second);
        }
        ...
        std::pair<size_t, size_t> findBinIndex(double coordX, double coordY) const {
            if (coordX < *_cachedBinEdges.first.begin() || 
                coordY < *_cachedBinEdges.second.begin() || 
                coordX >= *_cachedBinEdges.first.end() || 
                coordY >= *_cachedBinEdges.second.end()) {
        ...
        void scaleW(double scalefactor) {
            ...
            for (int i=0; i<_bins.size(); i++) {
                for (int j=0; j < _bins[i].size(); j++ {
                    _bins[i][j].scaleW(scalefactor);
                }
            }
        }
        // Operators:
        ...
        Axis2D<BIN>& operator += (const Axis2D<BIN>& toAdd) {
            ...
            for (int i=0; i < bins().size(); i++) {
                for (unsigned int j = 0; j < bins().at(i).size(); i++) {
        ...
        Axis2D<BIN>& operator -= (const Axis2D<BIN>& toSubstract) {
            ...
            for (int i=0; i < bins().size(); i++) {
                for (size_t j = 0; j < bins().at(i).size(); i++) {
                    bins().at(i).at(j) -= toSubstract.bins().at(i).at(j);
                }
            }
    ...
    private:
        ...
        std::pair<Utils::sortedvector<pair<double,Utils::sortedvector<pair<size_t, pair<double,double> > > > >,
                  Utils::sortedvector<pair<double,Utils::sortedvector<pair<size_t, pair<double,double> > > > > > 
                  _binHashSparse;
