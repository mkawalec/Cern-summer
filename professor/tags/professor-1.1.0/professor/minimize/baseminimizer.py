...
class BaseMinimizer(object):
    ...
    def setStartpoint(self, method="random", manualsp=None):
        ...
        if method == "center":
            ...
            self.__startpoint = ranges.getRelativePoint(
                                            numpy.random.rand(ranges.dim))
        elif method == "manual":
            if type(manualsp) == dict:
                ...
                TypeError("Given argument manualsp of wrong type:"
                          " %s" % (manualsp))

            if manualsp.dim != ranges.dim:
                raise ValueError("Manual start point has wrong dimension!")
            if manualsp.names != ranges.names:
                raise ValueError("Manual start point has wrong parameter"
                                 " names!")
        ...
        logging.info("new starting point: %s (method: %s)" % (
                     self.__startpoint, self.__startpointmethod))
    ...
    def fixParams(self, fixed, free=True):
        ...
        logging.info("Fixed parameters %s"%(self.__fixedparams))
