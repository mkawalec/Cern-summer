...
class ScanPointGenerator(PointGenerator):
    ...
    def getGradientsForExtremalScan(self, direction, symmetric):
        ...
        for k in sorted(p1.keys()):
            s += "\n        %s   %e   %e" % (k.ljust(maxlen), p1[k], p2[k])
        logging.info(s)
    ...
    def getPiercingPoints(self, direction, symmetric):
        ...
        t_lows= numpy.nan*numpy.ones(len(freeparamters))
        t_highs = numpy.nan*numpy.ones(len(freeparamters))
        for i, pname in enumerate(freeparamters):
            t_lows[i] = (self._paramranges[pname][0] - center[pname])/grad[pname]
            t_highs[i] = (self._paramranges[pname][1] - center[pname])/grad[pname]

        ## Test that this worked
        if numpy.isnan(t_lows).any() or numpy.isnan(t_highs).any():
            raise ValueError("Low or high parameterisations is NaN!")
        logging.trace("l:" + str(t_lows))
        logging.trace("h:" + str(t_highs))
...
class Hypercube(dict):
    ...
    def getGradient(self):
        """
        Return gradient of the line between points that define the hypercube.
        """
        gradient = {}
    ...
    def write(self, fname='hypercube.params'):
        ...
        for k in sorted(self.keys()):
            f.write("%s   %e   %e\n" % (k, self[k][0], self[k][1]))
        f.close()
        logging.info("Hypercube params written to %s" % fname)
...
class SymmetricHypercube(Hypercube):
    ...
    @classmethod
    def fromFlatFile(cls, ranges, flatpath):
        pp = parameter.ParameterPoint.fromFile(flatpath, ranges)
        return cls(ranges, pp.asDict(scaled=True))
