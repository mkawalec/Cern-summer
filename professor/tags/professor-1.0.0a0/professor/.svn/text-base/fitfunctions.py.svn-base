...
class SimpleIpolChi2(BaseChi2):
    ...
    def setParams(self, params, scaled=False):
        """Set the parameter point."""
        if type(params) == ParameterPoint:
            self.params = params
        elif type(params) == dict:
            self.params = ParameterPoint(params, self.tunedata.scaler, scaled)
        elif type(params) in (list, numpy.ndarray):
            self.params = ParameterPoint.fromList(params, self.tunedata.scaler, scaled)
        else:
    ...
    def calcChi2(self):
        ...
        for binprop in self.tunedata.filteredValues():
            err2 = 0.0
            simbin = binprop.ipol.getBin(self.params.getScaled())
            ## Reference error
