...
class MinimizationResult(parameter.FixedSortedKeys):
    ...
    ## TODO: all "builder" class methods should be called mkFoo, e.g. mkMRWithScaler
    @classmethod
    def withScaler(cls, gof, ndof, scaler, parscaled, **kwargs):
        ...
        new =  cls(gof, ndof, scaler.getKeys(), parscaled, parunscaled)
        # , errscaled,
    ...
    def __init__(self, gof, ndof, parnames, parscaled, parunscaled):
    ...
    def forParamFile(self):
        ...
        with, e.g. AGILe. Only a parameter's name and its value will be
        included.
        """
        temp = {}
...
class ResultList(list):
    ...
    def getMinimum(self):
        """Return minimizationresult that yields the smallest GoF value """
        def gof(mr):
            return mr.gof
    ...
    def write(self, outfile="results.pkl"):
        ...
        ## TODO: convert all dict fixed parameter info to sets
        """
        f=open(outfile,"w")
    ...
    def getUnique(self, goftol=1.e-10, paramtol=None):
        ...
        for res in self:
            ...
            if abs(cmpres.gof - res.gof) >= goftol:
                raise ValueError("GoF for runs '%s' differ to much:"
                                 " %r vs. %r"%(res.runs, res.gof, cmpres.gof))
