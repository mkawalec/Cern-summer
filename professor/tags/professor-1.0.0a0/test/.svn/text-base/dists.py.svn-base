...
class FakeDistribution(object):
    ...
    """
    def __init__(self, obs="FakeObs", binid="FakeObs:0", binrange=(0.0, 1.0),
            ydist=None, errdist=None):
        self._obs = obs
    ...
    def chi2Compare(self, interp, testpoints, error='fake dist'):
        ...
        @param error: keyword to set the error used. 'fake dist'(default),
            'interpolation', 'none' are possible.
        """
        chi2 = 0.0
        ...
        if error == 'fake dist':
            ...
            for tp in testpoints:
                chi2 += ( (self(tp).getYVal() - interp.getValue(tp.getScaled()))
                          / self._errdist.estimate(tp.getScaled()) )**2
            ...
            for tp in testpoints:
                chi2 += ( (self(tp).getYVal() - interp.getValue(tp.getScaled()))
                          / interp.getError(tp.getScaled()) )**2

        elif error == "none":
            for tp in testpoints:
                chi2 += (self(tp).getYVal() - interp.getValue(tp.getScaled()))**2
        else:
            raise ValueError("argument error must be 'fake dist',"
                    " 'interpolation', 'none' but is '%s'!"%(error))
    ...
    def pullCompare(self, interp, testpoints, error='fake dist'):
        ...
        if error == 'fake dist':
            ...
            raise ValueError("argument error must be 'fake dist' or"
                    " 'interpolation' or 'none' but is '%s'!"%(error))
...
class Paraboloid(YDist):
    ...
    def __str__(self):
        return '<Paraboloid: center=%s, scale=%s, offset=%e>' % (
            str(self._center).replace('\n', ''),
...
class OLDCubic(Quadratic):
    def __init__(self, cubmat, cubvec, quadmat, quadvec, quadoff):
        super(OLDCubic, self).__init__(quadmat, quadvec, quadoff)
    ...
    def __call__(self, p):
        ...
        return ( numpy.dot(self._cubvec, p**3)
                 ...
                 + super(OLDCubic, self).__call__(p)
                )

    def __str__(self):
        return "<OLDCubic: dim=%i>"%(self.getPShape()[0])


class Cubic(Quadratic):
    name = "Cubic"
    def __init__(self, cubtens, quadmat, linvec, offset):
        super(Cubic, self).__init__(quadmat, linvec, offset)
        dim = self.getPShape()[0]
        assert cubtens.shape == (dim, dim, dim)
        self._cubtens = cubtens

    def __call__(self, p):
        assert p.shape == self.getPShape()
        ndot = numpy.dot
        return (ndot(p, ndot(p, ndot(self._cubtens, p)))
...
class AbsoluteGaussianError(ErrorDist):
    ...
    def error(self, p_unused):
        return numpy.random.normal(0.0, self.sigma)
    ...
    def __str__(self):
        return "<AbsoluteGaussianError: sigma=%e>" % self.sigma


class RelativeGaussianError(ErrorDist):
    def __init__(self, relsigma, ydist):
        self.ydist = ydist
        self.relsigma = relsigma

    def error(self, p):
        return numpy.random.normal(0.0, abs(self.ydist(p)) * self.relsigma)

    def estimate(self, p):
        """Return the error estimate at the given point."""
        return self.ydist(p) * self.relsigma

    def __str__(self):
        return "<RelativeGaussianError: relsigma=%e>" % self.relsigma
