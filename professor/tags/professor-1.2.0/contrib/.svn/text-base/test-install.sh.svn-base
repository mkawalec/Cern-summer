...
"$PROFINTERPOLATE" --datadir "$EXC_DIR" --runs "$RUNSFILE" --outdir "$EXC_DIR" || { echo "Failed to run prof-interpolate" ; exit 1 ; } 
...
"$PROFTUNE" --datadir "$EXC_DIR" --runs "$RUNSFILE" --outdir "$EXC_DIR" || { echo "Failed to run prof-tune" ; exit 1 ; } 
