...
class EnvelopeGetter(object):
    ...
    envelopecolors = ["blue!40!", "red!40!", "blue!80!", "red!80!"]
    ...
    def getEnvelopeHisto(self, obs, cl=100.0, realcl=False):
        ...
        for ibin, bin0 in enumerate(histo0):
            ...
            out.addBin(Bin(xlow, xhigh,
                           yval = valmean[ibin],
                           yerrplus = valerr[ibin],
                           yerrminus = valerr[ibin]))
    ...
    def _getBinValues(self, obs):
        ...
        for irun, run in enumerate(self.mcdata.availableruns):
            ...
            for ibin, bin in enumerate(histo):
                values[ibin, irun] = bin.getYVal()
        return values

    def plotMakePlots(self, logy=False):
        if len(self._envelopes) == 0:
            raise RuntimeError("No envelope plots available. Call"
                               " addConfidenceLevel() first!")
        ...
        try:
            ...
            logging.debug("Reference data for envelope available for"
                          " %s" % (self.obs))
        except AttributeError, err:
            logging.warn("No reference data available in envelopes for"
                         " '%s'" % (self.obs))
            refhisto = None
        except DataProxyError, err:
            logging.error("Observable '%s' is missing in reference"
                          " data!" % (self.obs))
            refhisto = None
        ...
        for tune in self.tunes:
            try:
                ...
                logging.warn("Observable %s not available in own of the"
                             " tunes!" % self.obs)
