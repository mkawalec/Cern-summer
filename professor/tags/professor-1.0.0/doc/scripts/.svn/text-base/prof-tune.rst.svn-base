...
produced and stored in `OUTFILE`. Each result contains the optimal
...
resulting histograms are saved in a plain :file:`histo-{xxx}.dat` file.
These files are created in `HISTOOUTDIR`, see :option:`--histo-outdir`.
The production of these histograms can be turned off with :option:`-I`.
...
hyper-cube as initial point::
    ...
    prof-tune --data /my/data --weights my.weights --runsfile mycombinations
...
.. cmdoption:: --outfile OUTFILE
    ...
    Store results in `OUTFILE`. [default: :file:`results.pkl`]

.. cmdoption:: --histo-outdir HISTOOUTDIR

    Store plain :file:`histo-{xxx}.dat` histogram files as predicted by
    the parameterizations at the optimal parameter points in
    `HISTOOUTDIR`.
    [default: :file:`./ipolhistos/`]
