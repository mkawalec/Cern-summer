...
from baseminimizer import BaseMinimizer, MinimiserError
...
class PyMinuitMinimizer(BaseMinimizer):
    ...
    def initMinimization(self):
        ...
        for i, mpname in enumerate(mpnames):
            realname = self.tunedata.scaler.getKeys()[i]
            ...
            if self.getFixedParameter(realname) is not None:
                ...
                logging.debug("Fixing minuit parameter %s (%s) to %f" % (
            ...
            if self.getParameterLimits(realname) is not None:
                ...
                logging.debug("Limitting minuit parameter %s (%s) to %s" % (
                                mpname, realname, self.getParameterLimits(realname)))
        ...
        while True:
            ...
            try:
                ...
                if tries < self.maxtries:
                    logging.error("MIGRAD call #%i failed with: %s" % (
                                  tries, err))
                    logging.info("Current parameter values: %s" % (
                                 self.__minuit.values))
                    logging.error("Trying again...")
                else:
                    logging.error("Reached maximal numer of tries %i, but"
                                  " MIGRAD failed: Raising error!" % (
                                      self.maxtries))
