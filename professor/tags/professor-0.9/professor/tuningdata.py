...
class TuningData(object):
    ...
    def getMCHisto(self, histname, runnum):
        try:
            ...
            if self._mchistos.has_key(histname):
                print self._mchistos[histname].keys()
            print histname, runnum
            raise err
    ...
    def getTuneData(self, ipolcls, use_runnums=None, use_obs=None):
        ...
        @param use_obs : list of observable name strings. It may contain
                         zero-weighted bins, these are left out in the GoF
                         calculation in SingleTuneData.
        ...
        for obs in use_obs:
            try:
                ...
                logging.error("could not get ref histo for %s" % (obs))
    ...
    def getGoFFromMC(self, run, weightdict, extraveto=None):
        ...
        for obsname, val in weightdict.iteritems():
            weight = val[0]
            low = val[1]
            high = val[2]
            try:
                ...
                logging.debug("Requested observable %s not in TuningData"
                              " object!" % (obsname))
                continue
            for ibin, refbin in enumerate(self.getRefHisto(obsname).getBins()):
                bin = mchist.getBin(ibin)
                binid = self.getBinID(mchist, ibin)
                if refbin.getYErr() <= 0.0:
                    weight = 0.0
                    logging.debug("vetoing bin %s because ref. err <= 0.0" % (binid))
                if (weight and
                        extraveto is not None and
                        self.getBinID(mchist, ibin) in extraveto):
                    logging.debug("vetoing bin %s because of extraveto" % (binid))
                    weight = 0.0
                if weight:
                    bincenter = bin.getBinCenter()
                    if (low is not None and high is not None
                            and (bincenter < low or bincenter > high)):
                        weight = 0.0
                        logging.debug("vetoing bin %s because of low and high"
                                      " weighting edges" % (binid))
                    elif low is not None and bincenter < low:
                        weight = 0.0
                        logging.debug("vetoing bin %s because of low"
                                      " weighting edge" % (binid))
                    elif high is not None and bincenter < high:
                        weight = 0.0
                        logging.debug("vetoing bin %s because of high"
                                      " weighting edge" % (binid))
                gofdata.append((refbin, bin, weight))
...
class SingleTuneData(dict):
    ...
    def vetoZeroCoeffs(self):
        """Veto bins with interpolations that have only 0 coefficients."""
        for bp in self.itervalues():
            # if bp.refbin.getYErr() == 0 or bp.refbin.getYVal() == 0:
            #EIKE: this should test if all coeffs are 0.0
            # if bp.ipol.coeffs.all() == numpy.zeros(len(bp.ipol.coeffs)).all():
            if (bp.ipol.coeffs == 0.0).all():
                bp.veto = True
    ...
    def vetoEmptyErrors(self):
        ...
        for bid, bp in self.iteritems():
            # if bp.refbin.getYErr() == 0 or bp.refbin.getYVal() == 0:
            if bp.refbin.getYErr() <= 0.0:
                logging.warn("Vetoing bin %s because ref. error <= 0.0." % (bid))
                bp.veto = True
