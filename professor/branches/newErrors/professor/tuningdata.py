...
class SimpleGoFData(list):
    """Data container for chi^2/goodness of fit calculation.

    It's a list of BinProps instances combined with a GoF attribute
    (SimpleGoFData.gof).

    This class provides a simple weighted chi^2 as GoF measure. For the
    error estimate the error of the reference data is used.

    To use other GoF methods create subclasses of this class and overwrite
    the calcChi2 method and/or gof property.
    """
    def __init__(self, nopar, params=None, runnum=None):
        self.__nopar = nopar
        # check that only param or runnum is given
        if params is not None and runnum is not None:
            raise ValueError("Only argument 'params' or argument 'runnum'"
                    " can be specified!")
        if params is None and runnum is None:
            raise ValueError("Either argument 'param' or argument 'runnum'"
                    " must be specified!")
        # set helper function used in chi^2 calculation to get the bin
        # content of the simulation bin
        if params is not None:
            self.params = params
            self._getSimBinVal = self._getIpolSimBinVal
            self._getSimBin = self._getIpolSimBin
        else:
            self.runnum = runnum
            self._getSimBinVal = self._getMCSimBinVal
            self._getSimBin= self._getMCSimBin
    ...
    def calcNdof(self):
        ...
        # for binprop in self:
            # sumw += binprop.w
        sumw = sum(map(operator.attrgetter("weight"), self))
        return sumw - self.nopar
    ndof = property(calcNdof)

    def _getMCSimBinVal(self, binprop):
        return binprop.mcdict[self.runnum].getYVal()

    def _getIpolSimBinVal(self, binprop):
        return binprop.ipol.getValue(self.params, error=False)

    def _getMCSimBin(self, binprop):
        return binprop.mcdict[self.runnum]

    def _getIpolSimBin(self, binprop):
        return binprop.ipol.getBin(self.params, error=False)
    ...
    def calcChi2(self):
        ...
        for binprop in self:
            rchi2 += (
                binprop.weight
                * ( (binprop.refbin.getYVal() -
                    self._getSimBinVal(binprop) )
                / binprop.refbin.getYErr())**2 )
        return rchi2
    chi2 = property(calcChi2)
    gof = chi2


class OLDGoFData(SimpleGoFData):
    """Data container for chi^2/goodness of fit calculation.
    ...
    def __init__(self, nopar, referror=True, simerror=False,
            params=None, runnum=None):
        super(OLDGoFData, self).__init__(nopar, params, runnum)
    ...
    def calcChi2(self):
        ...
        for binprop in self:
            ...
            simbin = self._getSimBin(binprop)
...
class MCStatErrGoFData(SimpleGoFData):
    def __init__(self, nopar, params=None, runnum=None):
        super(MCStatErrGoFData, self).__init__(nopar, params, runnum)
        self.mcdisbelief = 0.0

    def calcChi2(self):
        logging.debug("Calculating MCStatErr chi^2 with MC disbelief"
                " factor %g" % (self.mcdisbelief))
        rchi2 = 0.0
        for binprop in self:
            num = (binprop.refbin.getYVal() - self._getSimBinVal(binprop))**2
            denom = (binprop.refbin.getYErr()**2
                    + binprop.mcstaterr2
                    + (self.mcdisbelief*self._getSimBinVal(binprop))**2)
            rchi2 += binprop.weight * num / denom
        return rchi2
    chi2 = property(calcChi2)
    gof = chi2


class CentralMCStatErrGoFData(SimpleGoFData):
    centralrun = None
    def calcChi2(self):
        rchi2 = 0.0
        for binprop in self:
            num = (binprop.refbin.getYVal() - self._getSimBinVal(binprop))**2
            denom = binprop.refbin.getYErr()**2
            if self.centralrun is not None:
                denom += binprop.mcdict[self.centralrun].getYErr()**2
            rchi2 += binprop.weight * num / denom
        return rchi2
    chi2 = property(calcChi2)
    gof = chi2
...
class TuningData(object):
    ...
    def getMCBinRanges(self, adict):
        """ loop over all mc-runs in a run:histo-dictionary and return
            the arrays of bin ranges again stored in another array
        """
        return numpy.array([self.getBinRanges(histo) for histo in
            adict.values()])

    def allMCandRefHaveSameBinRanges(self, obs):
        """ compare the individual binranges of all mc-runs with the binranges
            of the ref-histo and return True if they all match, only
        """
        refbinrange = self.getBinRanges(self.getRefHisto(obs)),
        mcbinranges = self.getMCBinRanges(self.getMCHistos(obs))
        for array in mcbinranges:
            if (array - refbinrange).all() == numpy.zeros(
                    (len(refbinrange), 2)).all():
                continue
            else:
                return False
        return True
    ...
    def isValid(self):
        ...
        for obs in self.getMCHistoNames():
            if self.allMCandRefHaveSameBinRanges(obs):
                continue
            else:
                raise TuningDataError("MC-binranges and ref binrange differ"
                        " in Observable %s!"%(obs))
    ...
    def getGoFFromMC(self, run, weightdict, extraveto=None):
        ...
        for obsname, weight in weightdict.iteritems():
            try:
                ...
                _logger.debug("Requested observable %s not in TuningData object!"%obsname)
                continue
    ...
    # TODO: make the interpolation cache working for multiple interpolation
    # classes
    def getInterpolationSet(self, ipolmethod, use_runnums, use_obs=None):
        ...
        # Build and cache a new interpolation set, if needed.
        ...
        ppcache = {}
        for runnum in use_runnums:
            ppcache[runnum] = ParameterPoint(self.getParams(runnum), scaler, scaled=False)
        ...
        try:
            ...
            return ripols
        # Eike: I'm not sure if we should drop this error here.
        #       Changed behaviour to log and then reraise the error.
        except ValueError, e:
    ...
    def getMostCentralRun(self, runs=None, count=1):
        """
        Return the count run number(s) with the most central parameter
        points.
        """
        if runs is None:
            runs = self.getRunNums()
        scaler = self.getScaler(runs)
        center = 0.5 * numpy.ones(scaler.dim())
        dists = numpy.zeros(len(runs))

        runs = sorted(runs)
        for i, runnum in enumerate(runs):
            pp = ParameterPoint(self.getParams(runnum), scaler, scaled=False)
            d = pp.getScaled() - center
            dists[i] = numpy.sqrt(numpy.dot(d,d))
        imins = dists.argsort()[:count]

        ret = [runs[i] for i in imins]
        # ret = []
        # for i in imins:
            # ret.append(runs[i])
        return ret
...
class WriteBackTuningData(TuningData):
...
class SingleTuneData(dict):
    ...
    numbers, observables and additional weights and vetos.
    """
    def __init__(self, refbins, mcbins, ipolset, extraveto=None, gofcls=SimpleGoFData):
        """
        ...
        @param gofcls: the class used for GoF calculations in getGoFFrom* methods,
            can be changed after creation by setting the SingleTuneData.gof property
        """
        t = {}
    ...
    def getGoFFromMC(self, run):
        ...
        logging.debug("Using %s with mcdisbelief=%f" % (self.gofcls.__name__,
            self.mcdisbelief))
        gofdata = self.gofcls(self.numParams(), runnum=run)
        if self.mcdisbelief:
            gofdata.mcdisbelief = self.mcdisbelief
        gofdata.extend(self.filteredValues())
        return gofdata
    ...
    def getGoFFromIpol(self, params, scaled=False):
        ...
        gofdata = self.gofcls(self.numParams(), params=pp.getScaled())
        if self.mcdisbelief:
            gofdata.mcdisbelief = self.mcdisbelief
        gofdata.extend(self.filteredValues())
        return gofdata
    ...
    def getInterpolationHisto(self, observable, parampoint, title=None):
        ...
        if not observable in self.getObservables():
            raise StandardError("Observable %s not amongst these:"%observable)
            print self.getObservables()

        if isinstance(parampoint, MinimizationResult):
            pointasarray = parampoint.parscaled
        else:
            if not isinstance(parampoint, ParameterPoint):
                try:
                    pointasarray = ParameterPoint(parampoint, self.scaler)._scaled
                except:
                    raise TypeError("parampoint has to be either a dict or a "
                            + "ParameterPoint instance!")
            else:
                pointasarray = parampoint
...
class BinProps(object):
    ...
    reference value and errors, a collection of its simulated equivalents
    from a set of MC runs, and an interpolation function for that bin, based
    on optimising the fit to a sampling of MC points in the parameter space.
    ...
    At the moment the following is stored:
     ...
     - mcstaterr/mcstaterr2: the mean of the statistical MC errors in
            quadrature:
                mcstaterr2 = sum(\Delta^2_MC)/N_MC = sum(MC)/N_MC
                mcstaterr = sqrt(mcstaterr2)
    """
    def __init__(self, refbin, mcdict, ipol):
        ...
        self.__mcstaterr = None
        self.__mcstaterr2 = None

    def _initMCStatError(self):
        self.__mcstaterr2 = 0.0
        for mcbin in self.mcdict.itervalues():
            self.__mcstaterr2 += mcbin.getYVal()
        self.__mcstaterr2 /= len(self.mcdict)
        self.__mcstaterr = numpy.sqrt(self.__mcstaterr2)

    def getMCStatError(self):
        if self.__mcstaterr is None:
            self._initMCStatError()
        return self.__mcstaterr

    def getMCStatErrorSqr(self):
        if self.__mcstaterr2 is None:
            self._initMCStatError()
        return self.__mcstaterr2

    mcstaterr = property(getMCStatError)
    mcstaterr2 = property(getMCStatErrorSqr)
