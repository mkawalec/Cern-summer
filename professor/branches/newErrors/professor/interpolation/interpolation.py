...
class BaseBinInterpolation(object):
    ...
    def getBin(self, p, error=True):
        if error:
            ...
            return Bin(xlow=self.binrange[0], xhigh=self.binrange[1],
                    yval=self.getValue(p, error=False), yerr=0.0)
...
class QuadraticBinInterpolation(BaseBinInterpolation):
    ...
    def getCoefficient(self, i1=None, i2=None):
        ...
        @raise InterpolationError: if i1 > i2 or i1, i2 >= npar.
        """
        if i1 is None and i2 is None:
            ...
            if i1 >= self.npar:
                raise InterpolationError("Index i1 must be less than"
            ...
            if i1 >= self.npar:
                raise InterpolationError("Index i1 must be less than"
                                         " the number of parameters")
            if i2 >= self.npar:
                raise InterpolationError("Index i2 must be less than"
                                         " the number of parameters")
            ind = 1 + self.npar
