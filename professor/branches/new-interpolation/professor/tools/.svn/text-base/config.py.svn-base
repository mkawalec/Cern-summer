...
class Config(object):
    ...
    def __singleton_init(self):
        ...
        self.__stderr.setFormatter(logging.Formatter(
                        '%(name)-12s: %(levelname)-8s %(message)s'))

        # init global stuff e.g. logging fall back values
        self.initModule('global', {'logfiles' : 'stderr', 'loglevel' : 'debug'})
    ...
    def initModule(self, module, moddict):
        ...
        Option Values:
            1. A single string
            2. A list with lengt between 1 and 4. List items are used as
               following:
                1. option value
    ...
    def initOption(self, module, name, val,
        ...
        @param val: the option value in the native type, e.g. C{int}, C{float}
        ...
        @param cmdline: If not None, a string used as option string. Must
            not begin with '--'. The user must take car that it's valid for
            use with optpars.OptionParser . If not a string type but
