...
class ROOTMinuitMinimizer(BaseMinimizer):
    ...
    def minimize(self):
        ...
        for i in xrange(self.__f.GetNpar()):
            ...
            if pi is not None:
                _logger.debug("fixing parameter #%i to %f"%(i, pi))
                self.__f.FixParameter(i, pi)
            else:
                _logger.debug("releasing parameter #%i"%(i))
        ...
        for i in xrange(scaler.dim()):
            ...
            _logger.debug("#%i %e +- %e (MIGRAD)"%(i, ival, ierr))
            _logger.debug("    MINOS    +- %e"%(eparab))
            _logger.debug("    MINOS    + %e  - %e"%(eplus, eminus))
            _logger.debug("    MINOS    gcc: %e"%(gcc))

        _logger.debug("chi^2: %s  ndf: %s"%(self.__f.GetChisquare(),
                                            self.__f.GetNDF()))
