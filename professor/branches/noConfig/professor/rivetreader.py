...
def addDataCLOptions(parser):
    ...
    group.add_option("--datadir",
            help = "specifiy directory containing mc, ref and ipol"
                   " directories")
    ...
    group.add_option("--ipoldir",
            help = "specify directory containing previously calculated"
                   " interpolations")
    ...
    for rawline in obsfile:
        ...
        try:
            ...
            if len(groups) == 2:
                if gettitles:
                    if groups[1] == None:
                        weightdict[groups[0]] = (1.0, title)
                    else:
                        weightdict[groups[0]] = (float(groups[1]), title)
                else:
                    if groups[1] == None:
                        weightdict[groups[0]] = 1.0
                    else:
                        weightdict[groups[0]] = float(groups[1])
            else:
                Exception("Catched right below and re-raised:)")
        except:
            raise ValueError("'%s' is not a valid observable file: could not"
                             " parse line '%s'"%(path, rawline))
    return weightdict
