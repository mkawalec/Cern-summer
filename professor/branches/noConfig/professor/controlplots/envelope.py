...
class Envelope:
    ...
    def __init__(self, td, observable=-1, plotwhat=(True, True, False),
        ...
        #logger.debug("now processing %s"%self._obs)
    ...
    def getPatch(self, xlow, xhigh, logy='no'):
        ...
        if logy != 'no' and (vmax <=0 or vmin <=0 or vpemax <=0 or vpemax_low <=0):
            logger.warning("encountered bad bin at %f...%f, vmax=%f, vmin=%f, vpemax=%f, vpemax_low=%f"%(xlow, xhigh, vmax, vmin, vpemax, vpemax_low))
