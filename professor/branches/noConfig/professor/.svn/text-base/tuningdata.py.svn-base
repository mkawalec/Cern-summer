...
class TuningData(object):
    ...
    def getGoFFromMC(self, run, weightdict, extraveto=None):
        ...
        for obsname, weight in weightdict.iteritems():
            try:
                ...
                print "Requested observable %s not in TuningData object!"%obsname
                continue
    ...
    def setInterpolationSet(self, ipolset):
        ...
        if self._ipols.has_key(runskey):
            logging.warn("Interpolations for runs %s already set!"
                    " Overwriting previous interpolations!"%(runskey))
        self._ipols[runskey] = ipolset

    # TODO: make the interpolation cache working for multiple interpolation
    # classes
    def getInterpolationSet(self, ipolcls, use_runnums, use_obs=None):
        ...
        """
        # import professor.interpolation as profipol
        if use_obs is None:
            use_obs = self.getMCHistoNames()
    ...
    def writeInterpolationSets(self, ipoldir):
        ...
        for runskey, ipolset in self._ipols.iteritems():
            #runs = set(runskey.split(":"))
            #fname = "%i-of-%i-missing"%(len(runs), len(allruns))
            #missing_runs = sorted(allruns - runs)
            #if len(missing_runs) > 0:
                #for r in sorted(allruns - runs):
                    #fname += "-%s"%(r)
            #else:
                #fname += "-none"
            #fname += ".xml"
            if not ipolset.getWriteFlag():
                ...
                logging.debug("storing interpolation set in '%s'"%(fname))
                ipolset.write(os.path.join(ipoldir, fname))
            else:
                logging.debug("ipolset already exists on hardrive")
...
class SingleTuneData(dict):
    ...
    """
    def __init__(self, refbins, mcbins, ipolset, extraveto=None):
        """
    ...
    def filteredValues(self):
        """Return an iterator with the bin properties without vetoed bins."""
        return itertools.ifilterfalse(lambda bp: bp.veto, self.itervalues())
