        ...
        for i, binid in enumerate(subset.sortedBinIDs()):
            ...
            for j, par in enumerate(parbincenters):
                ...
                if self._tundat is not None:
                    if self._tundat[binid].refbin.getVal() <= 0.0:
                        sens = 0.0
                        prof.log.debug("Sensitivity set to 0 for bin %s"
                                       " (refval=0)" % (binid))
                    else:
                        relreferr = (self._tundat[binid].refbin.getErr() /
                                     self._tundat[binid].refbin.getVal())
...
if __name__ == "__main__":
    ...
    def testSens(binfunc, parranges):
        ...
        ipol = InterpolationClass(order="cubic")
        OBS = "/My/Obs"

        sampler = RandomPointGenerator(parranges)
        sampler.seed(1)
        points = [p for p in
                    sampler.generate(int(ipol.minNumOfRuns(parranges.dim) * 1.5))]
